...
drums = ((0, 22), (22, 50), (50, 89), (89, 123),
         (123, 173), (173, 179), (179, 184))
SMALLTOM = 0
BIGTOM = 1
FLOORTOM = 2
SNARE = 3
...
# myMQTTClient.configureEndpoint("YOUR.ENDPOINT", 443)
myMQTTClient.configureCredentials("/greengrass/certs/AmazonRootCA1.pem",
                                  "/greengrass/certs/6acf979319.private.key", "/greengrass/certs/6acf979319.cert.pem")
...
def playYaml(yamlFile, sessionId, overrideTempo=0, duration=30.0, stageName="Guest"):
    ...
    with open(yamlFile, 'r') as stream:
        try:
            ...
            start_count(pixels, blink_times=1, color=GREEN,
                        tempo=tempo, sessionId=sessionId)
...
def processBar(beatCount=1, item=[], sessionId="123", tempo=120, duration=30.0, startTime=0, stageName="Guest"):
    ...
    beatDuration = time.time() - beatStart
    print("beatDuration: ", beatDuration)
    sleepTime = sleep - beatDuration
...
def sendReferenceData(sessionid, voltage, tz, epoch, drum, stageName):
    ...
    #payloadData['timestamp'] = (datetime.now(
    #    timezone('America/Chicago')) - epoch).total_seconds() * 1000.0
...
def blinkDrum(drum):
    for k in range(drum.startLED, drum.endLED):
        pixels.set_pixel(k, Adafruit_WS2801.RGB_to_color(
            drum.color[0], drum.color[1], drum.color[2]))
...
async def fireStickOne(ser, delay, drumA):
...
async def fireStickTwo(ser, delay, drumB):
...
def fireDrumstick(counter, ser, delay, drumA, drumB):
    ...
    if(counter % 2 == 0):
        asyncio.run(fireStickOne(ser, delay, drumA))
    else:
        asyncio.run(fireStickTwo(ser, delay, drumB))
    return
